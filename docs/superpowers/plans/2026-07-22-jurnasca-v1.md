...
- [x] **Step 1: Scaffold the app** — DONE (commits `dcf5a5f`, `10a2440`, `380e64a`)

Run:
```bash
npx create-next-app@latest . --typescript --tailwind --eslint --app --src-dir --import-alias "@/*"
```
The installed version (Next.js 16.2.11 + Tailwind v4) no longer prompts about Turbopack — it is the default dev/build engine now and this project accepts that default. Tailwind v4 also has no standalone `tailwind.config.ts`; it auto-detects content via `@import "tailwindcss";` in `globals.css` — do not create a `tailwind.config.ts` file, it would be dead code.

- [x] **Step 2: Replace the home page with a redirect to `/login`** — DONE
...
- [x] **Step 3: Verify the app builds and runs** — DONE
...
**Note on revalidation:** `revalidatePath` is called after every successful mutation so the `/staff/canales` server component re-fetches on the client's next `router.refresh()` — in this project's Next.js 16 + Turbopack dev setup, `router.refresh()` alone does not reliably re-render a server component with freshly mutated data (discovered and fixed in Task 11; see that task's report for the full investigation). Task 12's existing tests only exercise validation-failure paths (return before the `try` block), so no `next/cache` mock is needed in `canales.test.ts` — `revalidatePath` is never reached by those tests.
...
**Note on revalidation:** see Task 12's note on why `revalidatePath` is needed here (Task 11's Next.js 16/Turbopack dev-mode finding). Task 13's existing tests only exercise validation-failure and Prisma-rejection paths (both return before or without ever reaching the post-`create` `revalidatePath` line), so no `next/cache` mock is needed in `parcelas.test.ts`.
...
**Note on revalidation:** see Task 12's note on why `revalidatePath` is needed here (Task 11's Next.js 16/Turbopack dev-mode finding). Unlike Tasks 12-13, this task's own test file DOES exercise a successful mutation path (`cerrarPeriodoDirectivo`'s "closes a VIGENTE period" test), so the `vi.mock("next/cache", ...)` block above is required for that test to pass once `revalidatePath` is added to the implementation.
